# -*- coding: utf-8 -*-
# ============ anima_quick_template.py · anima 快速模式引子（黑盒，agent 只传参数） ============
# 与 anima_quick_template.json 同 basename → 自动配对。
# 用法：
#   python anima_quick_template.py --prompt "…" [--w 512 --h 768 --name 名字 --loras 名:强度:on]
# 只输出一行 JSON。anima 无 Seed 节点（靠分辨率破缓存）。
...
P_PROMPT, P_RES, P_PREFIX, P_LORA = "23", "19", "40", "52"


def build(prompt, w, h, name, lora_specs):
    wf = json.loads(TEMPLATE.read_text(encoding="utf-8"))
    wf[P_PROMPT]["inputs"]["string"] = prompt          # StringConstantMultiline → string
...
def main():
    p = argparse.ArgumentParser(description="anima 快速模式引子（×1.5 放大链：最终=初始×1.5）")
    p.add_argument("--prompt", required=True, help="人设二提示词（纯英文，六层结构）")
    p.add_argument("--w", type=int, default=512, help="初始宽（最终×1.5，8 的倍数）")
    p.add_argument("--h", type=int, default=768, help="初始高（最终×1.5，8 的倍数）")
